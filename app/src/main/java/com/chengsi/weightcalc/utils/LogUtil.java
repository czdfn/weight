...
public final class LogUtil {
	...
	/**
	 * A {@link LogUtil.Tree} that delegates to all planted trees in the {@link #FOREST
	 * forest}.
	 */
	...
	/**
	 * A {@link LogUtil.Tree} for debug builds. Automatically infers the tag from the
	 * calling class.
	 */
	public static class DebugTree implements TaggedTree {
		private static final Pattern ANONYMOUS_CLASS = Pattern
				.compile("\\$\\d+$");
		...
		private void throwShade(int priority, String message, Throwable t) {
			...
			if(!GlobalConstants.IS_PRODUCTION_MODE){
				String filePath = FileConstants.ROOT_LOG_PATH;
	...
	/** A {@link LogUtil.Tree} which does nothing. Useful for extending. */
