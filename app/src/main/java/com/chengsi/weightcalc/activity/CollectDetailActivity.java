...
public class CollectDetailActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int index = getIntent().getIntExtra(KEY_DYNAMIC_INDEX, 0);
        viewPager.setCurrentItem(index);
        if (dynamicBeanList != null && dynamicBeanList.size() > index) {
            dynamicBean = dynamicBeanList.get(index);
            if (menuItem != null) {
                if (dynamicBean != null && dynamicBean.isCollected()) {
                    menuItem.setTitle("取消收藏");
                } else {
    ...
    protected void initDataSource() {
        ...
        JDHttpClient.getInstance().reqCollectList(this, curPageNo, new JDHttpResponseHandler<ListBaseBean<List<DynamicBean>>>(new TypeReference<BaseBean<ListBaseBean<List<DynamicBean>>>>() {
        }) {
    ...
    private void cancelCollect() {
        ...
        JDHttpClient.getInstance().reqDelCollect(this, bean.getId(), new JDHttpResponseHandler<String>(new TypeReference<BaseBean<String>>() {
            ...
            public void onRequestCallback(BaseBean<String> result) {
                ...
                isChanged = true;
                if (dynamicBean != null && bean.getId() == dynamicBean.getId()) {
                    menuItem.setEnabled(true);
                    if (result.isSuccess()) {
                        menuItem.setTitle("收藏");
                    } else {
